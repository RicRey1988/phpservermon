...
class User
{
    ...
    protected function newRememberMeCookie()
    {
        ...
        // set cookie
        setcookie('rememberme', $cookie_string, $this->rememberMeCookieOptions(time() + PSM_LOGIN_COOKIE_RUNTIME));
    }
    ...
    protected function deleteRememberMeCookie()
    {
        ...
        setcookie('rememberme', '', $this->rememberMeCookieOptions(time() - (3600 * 3650)));
    }

    /** @return array{expires:int,path:string,secure:bool,httponly:bool,samesite:string,domain?:string} */
    private function rememberMeCookieOptions(int $expires): array
    {
        $options = array(
            'expires' => $expires,
            'path' => '/',
        ...
        if (is_string(PSM_LOGIN_COOKIE_DOMAIN) && PSM_LOGIN_COOKIE_DOMAIN !== '') {
            $options['domain'] = PSM_LOGIN_COOKIE_DOMAIN;
        }
